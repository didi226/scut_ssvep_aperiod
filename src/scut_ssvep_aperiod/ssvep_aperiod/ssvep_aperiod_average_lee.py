...
def difference_of_two_max(mylist,i):
	...
	if i == top_two_positions[0] :
		result = mylist[i]-mylist[top_two_positions[1]]
	else:
		result = mylist[i] - mylist[top_two_positions[0]]
	return result
...
if __name__ == "__main__":
	...
	for psda_type in ["snr_hqy","snr_hqy_ave_re","snr_hqy_ave_get"]:
		...
		pd.DataFrame(snr).to_excel(save_path, index=False, header=False, sheet_name='snr')
		pd.DataFrame(dif_snr).to_excel(save_path, index=False, header=False, sheet_name='dif_snr')
	...
	snr, dif_snr = ssvep_classify(form_path_lee, info_path, pro_ica=True, filter_para=[3,40],
	                     reconstruct_="remove_aperiodic", reconstruct_type=2, classify_method="cca",freq_range=[3,40])
	...
	pd.DataFrame(dif_snr).to_excel(save_path, index=False, header=False, sheet_name='dif_snr')
	snr, dif_snr = ssvep_classify(form_path_lee, info_path, pro_ica=True, filter_para=[3, 40],
	                              reconstruct_="remove_aperiodic", reconstruct_type=2, classify_method="fbcca",freq_range=[3, 40])
	...
	pd.DataFrame(dif_snr).to_excel(save_path, index=False, header=False, sheet_name='dif_snr')
	snr, dif_snr = ssvep_classify(form_path_lee, info_path, pro_ica=True, filter_para=[3, 40],
	                              reconstruct_="remove_aperiodic", reconstruct_type=2, classify_method="tdca",freq_range=[3, 40])
